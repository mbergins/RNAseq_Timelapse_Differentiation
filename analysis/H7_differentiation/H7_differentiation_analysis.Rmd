---
title: "Time-lapse RNAseq H7 CCM Analysis"
...
```{r id_conversion, cache=T, warning=F}
...
Looks like some of the UCSC transcript IDs don't have HGNC names (`r no_HGNC_count` or `r sprintf('%0.1f',100*no_HGNC_count/length(DESeq_results$hgnc_symbol))`%). 

For reference, `r dim(signif_transcripts)[1]` transcripts have a p-value below 0.05. This represents `r length(unique(na.omit(signif_transcripts$hgnc_symbol)))` genes with an HGNC ID. Here is the p-value distribution for the transcript level data:
...
```{r top_five_gene_plots, warning=F}
...
There are `r length(unique(signif_kinases$hgnc_symbol))` unique kinases in this list of significantly differentially regulated kinases.
...
```{r top_five_kinase_plots, warning=F}
...
OK, now what if just look at the understudied kinases? For reference, there are `r dim(signif_kinases %>% filter(class=="Dark"))[1]` understudied kinases in the differentially regulated list.

```{r top_five_dark_kinase_plots, warning=F}
